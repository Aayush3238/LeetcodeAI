...
import { Calendar, Clock, CheckCircle, Circle, History } from 'lucide-react'
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-4">
      <div className="flex items-center gap-4 text-sm text-dark-400 text-gray-500">
        <span className="flex items-center gap-1"><Clock size={14} /> {plan.items?.length || 0} days</span>
        <Badge variant="primary">{plan.type}</Badge>
        {plan.createdAt && <span className="text-xs">{new Date(plan.createdAt).toLocaleDateString()}</span>}
      </div>
...
export default function RevisionPlan() {
  ...
  const { data: savedPlans, isLoading: loadingSaved } = useQuery({
    queryKey: ['revision-plans'],
    queryFn: () => aiAPI.getRevisionPlans().then((r) => r.data.plans),
  })
  ...
  const { mutate: generatePlan, isPending } = useMutation({
    ...
    onSuccess: (plan) => { setPlanData(plan); setViewSaved(null) },
  })

  const displayPlan = viewSaved || planData
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        {PLAN_TYPES.map((pt) => (
          <motion.button
            ...
            whileTap={{ scale: 0.98 }}
            onClick={() => { setSelectedPlan(pt.value); setViewSaved(null); generatePlan(pt.value) }}
            disabled={isPending}
            className={`card p-6 text-left transition-all ${selectedPlan === pt.value && !viewSaved ? 'border-primary-500 ring-1 ring-primary-500' : 'hover:border-dark-700 hover:border-gray-300'}`}
          >
      ...
      {displayPlan && !isPending && <PlanView plan={displayPlan} />}

      {!displayPlan && !isPending && (
        ...
        <div className="card p-6">
          ...
          <div className="space-y-2">
            {savedPlans.map((sp) => (
              <button
                key={sp.id}
                onClick={() => { setViewSaved(sp); setPlanData(null) }}
                className={`w-full text-left p-3 rounded-xl transition-colors flex items-center justify-between ${viewSaved?.id === sp.id ? 'bg-primary-600/10 border border-primary-600/20' : 'hover:bg-dark-800/50 hover:bg-gray-100'}`}
              >
                <div>
                  <span className="font-medium text-sm">{sp.type} plan</span>
                  <span className="text-dark-400 text-gray-500 text-xs ml-3">{sp.items?.length} days</span>
                </div>
                <span className="text-dark-500 text-gray-400 text-xs">{new Date(sp.createdAt).toLocaleDateString()}</span>
              </button>
            ))}
          </div>
        </div>
