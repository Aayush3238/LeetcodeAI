    ...
    <div className="space-y-6">
      ...
      <div className="card p-4">
        <div className="flex flex-col sm:flex-row gap-3">
          ...
          <select value={topic} onChange={(e) => { setTopic(e.target.value); setPage(1) }} className="input">
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-dark-800">
                ...
                <th className="text-left px-6 py-4 text-sm font-medium text-dark-400">Topic</th>
                <th className="text-left px-6 py-4 text-sm font-medium text-dark-400">Acceptance</th>
              </tr>
            </thead>
            <tbody>
              {isLoading ? (
                ...
                (data?.problems || []).map((p, i) => (
                  ...
                  >
                    <td className="px-6 py-4 text-dark-500 text-sm">{p.leetcodeId}</td>
                    ...
                    <td className="px-6 py-4 text-dark-400 text-sm">{p.acceptance}%</td>
                  </motion.tr>
                ))
              )}
            </tbody>
      ...
      {data && (
        <div className="flex items-center justify-between">
          <p className="text-dark-400 text-sm">Showing {((page - 1) * 15) + 1}-{Math.min(page * 15, data.total)} of {data.total}</p>
          <div className="flex items-center gap-2">
            <button onClick={() => setPage(Math.max(1, page - 1))} disabled={page === 1} className="btn-ghost p-2 disabled:opacity-50">
              <ChevronLeft size={18} />
            </button>
            <span className="text-sm text-dark-400">Page {page} of {data.totalPages}</span>
