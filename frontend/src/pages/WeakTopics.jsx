          ...
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="card p-6">
            ...
            <div className="space-y-3">
              {(data?.strongTopics || []).map((t) => (
                <div key={t.topic} className="flex items-center justify-between p-3 bg-dark-800/50 rounded-xl">
                  <span className="text-sm">{t.topic}</span>
                  <div className="flex items-center gap-2">
                    <div className="w-16 h-1.5 bg-dark-700 rounded-full overflow-hidden">
                      <div className="h-full bg-green-400 rounded-full" style={{ width: `${t.strengthScore}%` }} />
                    </div>
                    <span className="text-xs text-dark-400 w-8 text-right">{t.strengthScore}%</span>
                  </div>
                </div>
              ))}
            </div>
      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="card p-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {(data?.weakTopics || []).slice(0, 3).map((t) => (
            <div key={t.topic} className="p-4 bg-dark-800/50 rounded-xl space-y-2">
              <div className="flex items-center gap-2">
